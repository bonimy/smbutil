...
static HTREEITEM InsertRoomDependencyTreeViewItem(LPROOMINFO *lpRoomInfo,int *piCurRoom,BYTE** pbParentRoom,int iNumParentRooms,HWND hDlg,HTREEITEM hParentItem,HTREEITEM hPrevItem)
{
...

	{
		...
		if(hParentItem==TVI_ROOT)
			wsprintf(cStrBuf,TEXT("%d-%d (%d) [%.2xH]"),(*lpRoomInfo)[nCurRoom].iWorld+1,(*lpRoomInfo)[nCurRoom].iArea+1,(*lpRoomInfo)[nCurRoom].iAreaIndex,(*lpRoomInfo)[nCurRoom].bRoomID);
		else
			wsprintf(cStrBuf,TEXT("[%.2xH] p=%d"),(*lpRoomInfo)[nCurRoom].bRoomID,(*lpRoomInfo)[nCurRoom].iPage);
		tvi.pszText = cStrBuf;
...
#define CY_BITMAP 16
	...
	int n;
	LPTSTR lpImgResName[]={TEXT("SEA_IMG"),TEXT("SKY_IMG"),TEXT("UG_IMG"),TEXT("CASTLE_IMG")};

    // Create the image list. 
    if((himl = ImageList_Create(CX_BITMAP, CY_BITMAP,ILC_MASK, NUM_BITMAPS, 0)) == NULL) return FALSE; 

    // Add the open file, closed file, and document bitmaps. 
	hmask = LoadBitmap(GetModuleHandle(NULL),TEXT("TVMASK")); 
...
//	char cStr[100];
	...
	for(n=0;n<SMB_NUM_AREAS;n++){
		...
		wsprintf(szBuf,TEXT("%d-%d %.2xH %s"),iWorld+1,iArea+1,bAreaData[n],lpAttr[(bAreaData[n]>>5)&0x03]);
			...
			case IDOK:
				{
					...
					if(1!=_stscanf(cBuf,TEXT("%hhx"),&bRoomID)) return TRUE;
