...
#define BORDERWNDCLASSNAME  TEXT("BORDER")
#define LOGVIEWWNDCLASSNAME TEXT("LOGVIEW")
...
BOOL lv_Initialize()
{
	...
	lf.lfCharSet = LOGVIEW_CHARSET;
	lstrcat(lf.lfFaceName, LOGVIEW_FONTNAME); // lstrcat(lf.lfFaceName, "Arial");
...
BOOL lv_OutputString(LPTSTR szText, DWORD dwFlag)
{
	...
	if (dwFlag & LOGVIEW_OUTPUTSTRING_CR){
		lstrcat(g_szLogBuffer,TEXT("\x0D\x0A"));
	}
