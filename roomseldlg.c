       ...
       case WM_INITDIALOG:
		   {
			   ...
			   if(lpRoomSelect->blDoInit){
				   //
				   wsprintf(cBuf,TEXT("%.2x"),lpRoomSelect->bInitRoomID&0x7F);
				   SetDlgItemText(hDlg,IDC_DATA,cBuf);
				   //
				   wsprintf(cBuf,TEXT("%d"),lpRoomSelect->uInitPage);
...
BOOL RoomSelectDialogBox(HWND hWnd,LPROOMSELECT lpRoomSelect)
{
	...
	blRet=DialogBoxParam(GetModuleHandle(NULL),TEXT("SENDOBJECTDLG"),hWnd,RoomSelectDlgProc,(LPARAM)lpRoomSelect);
