...
static void DisableBadguysEditDlgControls(HWND hDlg,int iDisableFlag)
{
	...
	if(iDisableFlag==DISABLE_2BYTES)
	{
		...
		SetDlgItemText(hDlg,iEditCtrlID[n],TEXT(""));
...
void UpdateBadguysEditDlgPreview(HWND hDlg,BOOL blGetRoomIDFromList)
{
	...
	if(!blGetRoomIDFromList)
	{
		...
		if(1!=_stscanf(cBuf,TEXT("%hhx"),&bRoomID)) return;
	}
	else
	{
		int iSel;
       ...
       case WM_INITDIALOG:
		   {
			   ...
			   if((bBuf[0]&0x0F)==0x0E)
			   {
				   ...
				   wsprintf(cBuf,TEXT("%.2x %.2x %.2x"),bBuf[0],bBuf[1],bBuf[2]);
				   ...
				   //Room ID
				   wsprintf(cBuf,TEXT("%.2x"),bBuf[1]&0x7F);
...
//					   CheckDlgButton(hDlg,IDC_BIT6,BST_CHECKED);
				   ...
				   //bin
				   wsprintf(cBuf,TEXT("%.2x %.2x"),bBuf[0],bBuf[1]);
				   SetDlgItemText(hDlg,IDC_BIN,cBuf);
				   CheckDlgButton(hDlg,IDC_ISBIN,BST_UNCHECKED);
			   ...
			   case IDOK:
				   {
					   if(IsDlgButtonChecked(hDlg,IDC_ISBIN)&BST_CHECKED)
					   {
						   ...
						   iSize=_stscanf(cBuf,TEXT("%hhx %hhx %hhx %hhx"),&bBuf[0],&bBuf[1],&bBuf[2],&bBuf[3]);
...
//						   TCHAR cBuf[20];
						   ...
						   else
						   {
							   ...
							   //if(1!=_stscanf(cBuf,"%d",&bTmp)) return TRUE;
					   ...
					   else
					   {
						   ...
						   if(1!=_stscanf(cBuf,TEXT("%hhx"),&bTmp)) return TRUE;
						   bBuf[1]|=(bTmp&0x7F);
						   //ワールド
...
{
	SetDlgItemText(hDlg,IDC_LEN,TEXT(""));
			   ...
			   case IDOK:
				   {
					   ...
					   if(IsDlgButtonChecked(hDlg,IDC_ISBIN)&BST_CHECKED)
					   {
						   ...
						   iValidSize=_stscanf(cBinBuf,TEXT("%hhx %hhx %hhx"),&bBinBuf[0],&bBinBuf[1],&bBinBuf[2]);
