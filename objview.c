...
#define MAPVIEWWNDCLASSNAME TEXT("MAPVIEWWND")
	...
	case WM_RBUTTONDOWN:
	{
		...
		if(ObjviewMousePos2ObjPos(LOWORD(lParam), HIWORD(lParam), &MouseInput)){
			...
			if(PtInRect((LPRECT)&rc,pt))
				ShowPopupMenu(hWnd, pt,TEXT("MAPVIEW_POPUP"));
		}
	}
	break;
   case WM_MOUSEMOVE:
	...
	case WM_CREATE:
		{
			...
			ghRPBtnWnd=CreateWindow(
				        ...
				        TEXT(""),
						 ...
						 GetModuleHandle(NULL),
						 NULL);
			hBmp=(HBITMAP)LoadImage(GetModuleHandle(NULL),TEXT("LF_IMG"),IMAGE_BITMAP,0,0,LR_LOADMAP3DCOLORS);
			SendMessage(ghRPBtnWnd,BM_SETIMAGE,(WPARAM)IMAGE_BITMAP,(LPARAM)hBmp);		

			ghFPBtnWnd=CreateWindow(TEXT("BUTTON"),
				        TEXT(""),
						 WS_CHILD|WS_VISIBLE|BS_BITMAP|BS_CENTER,
						 ...
						 GetModuleHandle(NULL),
						 NULL);
			hBmp=(HBITMAP)LoadImage(GetModuleHandle(NULL),TEXT("RG_IMG"),IMAGE_BITMAP,0,0,LR_LOADMAP3DCOLORS);
...
BOOL RegisterMapViewWndClass(HINSTANCE hInstance)
{
    ...
    wc.hIcon = LoadIcon( hInstance, TEXT("MAPVIEWICON") );
