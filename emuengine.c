...
static BOOL LoadRAMData(LPTSTR ResName)
{
	...
	hRAMData=LoadResource(NULL,FindResource(NULL,ResName,TEXT("BINARY")));
...
void TestPlaySetup(TESTPLAYSETUP *psTPS)
{
	...
	//
	if(!LoadRAMData(TEXT("RAMDATA1"))) return;
...
void TestPlaySetupEx(TESTPLAYSETUPEX *psTPSEx)
{
	...
	if(!LoadRAMData(TEXT("RAMDATA2"))) return;
...
BOOL RegisterEmuWndClass(HINSTANCE hInstance)
{
    ...
    wc.hIcon = LoadIcon(hInstance, TEXT("EMUICON"));
