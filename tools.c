...
static BYTE ConverttChr2Data(TCHAR cChar,BOOL *blUnknown)
{
	BYTE bRet;
...
static void ChangeString(UINT iStringNum,LPTSTR pString)
{
	...
	for(i=0,n=0;i<smbStringData[iStringNum].bMaxLen;i++)
	{
		...
		bTmp=0x24;// space
		if(!blEOB){
			bTmp=ConverttChr2Data(*(pString+n),&blUnknown);
			if(blUnknown){
				TCHAR cTmp[4]={'\0'};
				memcpy(cTmp,&pString[n],3 * sizeof(TCHAR));
				_stscanf(cTmp,TEXT("#%hhx"),&bTmp);
				n+=3;
			}
			else
				n++;
...
static void GetString(UINT iStringNum,LPTSTR pString,UINT iBufSize)
{
	...
	memset(pString,0,iBufSize *sizeof(TCHAR));
	pbTmp= (smbStringData[iStringNum].bRomType)?bCHRROM:bPRGROM;
	pbTmp+=smbStringData[iStringNum].wOffset;
	for(i=0,n=0;(i<smbStringData[iStringNum].bMaxLen)&&(n<iBufSize);i++){
		tcTmp=ConvertData2TChar(pbTmp[i]);
		if(tcTmp=='?'){
			wsprintf(&pString[n],TEXT("#%.2X"),pbTmp[i]);
			n+=3;
		}
		else
			pString[n++]=tcTmp;
			   ...
			   case IDC_STRINGSELECT:
				   {
					   if(HIWORD(wParam)==CBN_SELCHANGE)
					   {
						   ...
						   memset(cBuf,0,(SMB_STRING_MAXCHARS+1) * sizeof(TCHAR));
       ...
       case WM_INITDIALOG:
		   {
               ...
               wsprintf(cBuf,TEXT("%.2x %.2x %.2x %.2x %.2x %.2x %.2x %.2x %.2x %.2x %.2x"),bTmp[0],bTmp[1],bTmp[2],bTmp[3],bTmp[4],bTmp[5],bTmp[6],bTmp[7],bTmp[8],bTmp[9],bTmp[10]);
			   ...
			   wsprintf(cBuf,TEXT("%.2x"),bPRGROM[SMBADDRESS_LOOP_W7DATA1]);
			   SetDlgItemText(hDlg,IDC_DATA1,cBuf);
			   wsprintf(cBuf,TEXT("%.2x"),bPRGROM[SMBADDRESS_LOOP_W7DATA2]);
			   ...
			   case IDOK:
				   {
					   ...
					   if(11!=_stscanf(cBuf,TEXT("%hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx"),&bTmp[0][0],&bTmp[0][1],&bTmp[0][2],&bTmp[0][3],&bTmp[0][4],&bTmp[0][5],&bTmp[0][6],&bTmp[0][7],&bTmp[0][8],&bTmp[0][9],&bTmp[0][10])) return TRUE;

                       memset(cBuf, 0, sizeof(cBuf));
					   GetDlgItemText(hDlg,IDC_PAGE,cBuf,34);
					   if(11!=_stscanf(cBuf,TEXT("%hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx"),&bTmp[1][0],&bTmp[1][1],&bTmp[1][2],&bTmp[1][3],&bTmp[1][4],&bTmp[1][5],&bTmp[1][6],&bTmp[1][7],&bTmp[1][8],&bTmp[1][9],&bTmp[1][10])) return TRUE;

                       memset(cBuf, 0, sizeof(cBuf));
					   GetDlgItemText(hDlg,IDC_YPOS,cBuf,34);
					   if(11!=_stscanf(cBuf,TEXT("%hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx"),&bTmp[2][0],&bTmp[2][1],&bTmp[2][2],&bTmp[2][3],&bTmp[2][4],&bTmp[2][5],&bTmp[2][6],&bTmp[2][7],&bTmp[2][8],&bTmp[2][9],&bTmp[2][10])) return TRUE;

                       memset(cBuf, 0, sizeof(cBuf));
					   GetDlgItemText(hDlg,IDC_RETURNPOS,cBuf,34);
					   if(11!=_stscanf(cBuf,TEXT("%hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx"),&bTmp[3][0],&bTmp[3][1],&bTmp[3][2],&bTmp[3][3],&bTmp[3][4],&bTmp[3][5],&bTmp[3][6],&bTmp[3][7],&bTmp[3][8],&bTmp[3][9],&bTmp[3][10])) return TRUE;

                       memset(cBuf, 0, sizeof(cBuf));
					   GetDlgItemText(hDlg,IDC_DATA1,cBuf,3);
					   if(1!=_stscanf(cBuf,TEXT("%hhx"),&bData[0]))return TRUE;

                       memset(cBuf, 0, sizeof(cBuf));
					   GetDlgItemText(hDlg,IDC_DATA2,cBuf,3);
					   if(1!=_stscanf(cBuf,TEXT("%hhx"),&bData[1])) return TRUE;
       ...
       case WM_INITDIALOG:
		   {
			   ...
			   int n;
			   ADDRESSDATA adKoopa;
			   extern struct {LPTSTR Name;int YDelta;BYTE bFixedYPos;int XDelta;}smbBudGuysInfo[];
